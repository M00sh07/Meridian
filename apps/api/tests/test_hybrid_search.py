...
def test_explicit_semantic_mode(test_data, mock_embedding_provider):
    ...
    # First chunk should have semantic score
...
def test_explicit_lexical_mode(test_data, mock_embedding_provider):
    ...
    assert response.status_code == 200
    results = response.json()["results"]
    # Chunk 2 has symbol "Database" and content "database connection"
...
def test_no_embeddings_lexical_works(test_data, mock_embedding_provider):
    ...
    # Semantic score will be 0 but lexical should still find "database"
...
def test_total_semantics(test_data, mock_embedding_provider):
    ...
    # Semantic total is embedded chunks
    response = client.get(f"/repositories/{repo_id}/search?q=test&mode=semantic")
    assert response.json()["total"] == 3  # 3 chunks with embeddings

    # Lexical and hybrid total is all chunks in repo (4 chunks total in test_data repo1)
...
def test_camel_case_pascal_case_scoring():
    ...
    assert score2 > 0.0 # Should get overlap points

def test_deterministic_tie_ordering(test_data, mock_embedding_provider):
    from database import SessionLocal
    db = SessionLocal()
    repo_id = test_data["repo1_id"]
...
def test_candidate_limiting_exact_match(test_data, mock_embedding_provider):
    from database import SessionLocal
    import os
    db = SessionLocal()
    repo_id = test_data["repo1_id"]
    ...
    # Exact match MUST be in the top results despite candidate limiting bounding to limit*2 = 4 candidates
